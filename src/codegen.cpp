...
int asmWrite() {
    ...
    while (irGetData(data, 1) == 1) {
        ...
        if (strcmp(data, "TRUE") == 0) {
            ...
            free(popStack(&polish)); // Ignoro el AND y OR, pues no generan codigo assembler
            // popStack(&polish);
        }
        // Operadores Binarios
            ...
            // popStack(&polish); //Quitar el operador de la lista
            free(popStack(&polish));
            aux1 = dequeue(&ASMElementsRead); // Consumir dos operandos
            ...
            if (strcmp(data, "CMP") == 0) {
                ...
                // TODO ver que accion tomo ante un OR en la polish, posiblemente debería ignorarlo
                // al igual que el AND y NOT
            }
        ...
        // Operadores Unarios
                 ...
                 strcmp(data, "JMP") == 0 || strcmp(data, "WRITE") == 0 ||
                 strcmp(data, "READ") == 0) {
            free(popStack(&polish));
            // popStack(&polish);
            aux1 = dequeue(&ASMElementsRead);
            // VER WIKIPEDIA https://en.wikipedia.org/wiki/Branch_(computer_science) PARA MAS INFO
            // SOBRE LOS BRANCH
            if (strcmp(data, "JL") == 0) {
            ...
            // printf("Apilar - ASMElementsRead: %s\n", data);
